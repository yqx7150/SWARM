...
def train(config, workdir):
  ...
  train_ds = datasets.get_dataset(config,
                                              uniform_dequantization=config.data.uniform_dequantization)
  ...
  #################################################
  # resize_small = filter_datasets_2c.resize_small
  #################################################
  ...
  for step in range(initial_step, num_train_steps + 1):
    ...
    if step % config.training.log_freq == 0:
      print("data load completed ! cost time : {:.3f}".format(time.time() - flag))    
    flag = time.time()
    # batch = scaler(batch)
    #########################################################
    # if config.data.image_size != 512:
    #   batch = resize_small(batch,512)
    #########################################################
    ...
    if step % config.training.log_freq == 0:
      print("Move to cuda completed ! cost time: {:.3f}".format(time.time() - flag))
    flag = time.time()
    # print(batch.shape)
    # assert False  
    loss = train_step_fn(state, batch)
    if step % config.training.log_freq == 0:
      print("backward completed ! cost time: {:.3f}".format(time.time() - flag))

    flag = time.time()
    if step % config.training.log_freq == 0:
      logging.info("step: %d, training_loss: %.5e" % (step, loss.item()))
      writer.add_scalar("training_loss", loss, step)
      print("add log scalar completed ! cost time: {:.3f}".format(time.time() - flag))
    # Save a temporary checkpoint to resume training after pre-emption periodically

    flag = time.time()
    if step != 0 and step % config.training.snapshot_freq_for_preemption == 0:
      save_checkpoint(checkpoint_meta_dir, state)
      print("save model completed ! cost time: {:.3f}".format(time.time() - flag))
    ...
    # if True:
      ...
      print("save checkpoint completed ! cost time: {:.3f}".format(time.time() - flag))
      ...
      print("sampling completed ! cost time: {:.3f}".format(time.time() - flag))
  ...
  for ckpt in range(begin_ckpt, config.eval.end_ckpt + 1):
    ...
    if config.eval.enable_sampling:
      ...
      for r in range(num_sampling_rounds):
        ...
        #=============================================================#
        samples = np.clip(samples.permute(0, 2, 3, 1).cpu().numpy(), -1, 1)
        samples_img = (samples[0,:,:,0]+1)/2
        cv2.imwrite('{}/sample.png'.format(this_sample_dir),255*samples_img)
        #=============================================================#
